  ...
  def get_info
    ...
    if user_type == 'dispatcher'
      render json: {
        ...
        appointments: current_user.dispatcher_patients.uniq.map{|patient| patient.patient_appointments}.flatten.as_json(except: [:created_at, :updated_at], include: [notes: {only: :content}])
      }, status: :ok
